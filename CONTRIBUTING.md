...
`src/meorphis_test_6/lib/` and `examples/` directories are exceptions and will never be overridden.
...
```bash
pip install git+ssh://git@github.com:meorphis-test/test-repo-6.git
```
...
You can release to package managers by using [the `Publish PyPI` GitHub action](https://www.github.com/meorphis-test/test-repo-6/actions/workflows/publish-pypi.yml). This will require a setup organization or repository secret to be set up.
